...
func (e *MockSuite) TestMock(c *check.C) {
	...
	instances.Update("i-1", &v2.AzureInterface{ID: "intf-1"})
	api.UpdateInstances(instances)
	instances, err = api.GetInstances(context.Background())
	c.Assert(err, check.IsNil)
	c.Assert(len(instances), check.Equals, 1)
	c.Assert(instances.Get("i-1")[0], checker.DeepEquals, &v2.AzureInterface{ID: "intf-1"})
	...
	c.Assert(err, check.IsNil)
	c.Assert(len(instances), check.Equals, 1)
	c.Assert(instances.Get("i-1")[0], checker.DeepEquals, &v2.AzureInterface{
		ID: "intf-1",
		Addresses: []v2.AzureAddress{
